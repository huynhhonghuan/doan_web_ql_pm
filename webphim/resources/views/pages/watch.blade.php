                            ...
                            <div role="tabpanel" class="tab-pane active server-1" id="server-0">
                                <div class="halim-server">
                                    <ul class="halim-list-eps">
                                        @foreach ($movie->episode as $key => $epp)
                                            <a href="{{url('xem-phim/'.$movie->slug.'/tap-'.$epp->episode)}}">
                                                <li class="halim-episode"><span
                                                        class="halim-btn halim-btn-2 {{ $key == $tapphim-1 ? 'active' : '' }} halim-info-1-1 box-shadow"
                                                        data-post-id="37976" data-server="1" data-episode="1"
                                                        data-position="first" data-embed="0"
                                                        data-title="Xem phim {{ $movie->title }} - Tập {{ $epp->episode }} - Be Together - vietsub + Thuyết Minh"
                                                        data-h1="{{ $movie->title }} - tập {{ $epp->episode }}">{{ $epp->episode }}</span>
                                                </li>
                                            </a>
                                        @endforeach
                                    </ul>
